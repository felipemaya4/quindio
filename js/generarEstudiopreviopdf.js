/// funcion para generar el documento en formato pdf del estudio previo
async function generardocumento(numeroPrecontractual) {
    const result = await getEstudioPrevio(numeroPrecontractual);
    if(result.length == 0){
        alert("estudio no encontrado");
    }
    else{

        const contenidoFormulario = result[0].contenido;
        let obligaciones = contenidoFormulario.listaObligaciones;
        const listaDeObligacionesAMostrar = [];
        // se formatea el valor de cada obligacion para mostrarlo en el documento de forma estilizada segun libreria pdfmake
        obligaciones.forEach((text) => {
            listaDeObligacionesAMostrar.push({ text: [{ text: text, bold: false }, { text: '\n' }] })
        });
            ...
            content: [
                ...
                },
                {
                    ol: [
                            ...
                            ol: [

                                {
                                    ...
                                    { text: contenidoFormulario.tipoContrato.toUpperCase(), bold: true },/// insercion de datos del formulario
                                        ' nominado el numeral 3º del artículo 32 de la Ley 80 del año 1993, como también nominado en el artículo 2.2.1.2.1.4.9 del Decreto 1082 del año 2015, el cual determina: ', {
                                        text: '"Los servicios profesionales y de apoyo a la gestión corresponden a aquellos de naturaleza intelectual diferentes a los de consultoría que se derivan del cumplimiento de las funciones de la Entidad Estatal, así como los relacionados con actividades operativas, logísticas, o asistenciales".\n\n',
                                        italics: true
                                    }
                                    ], bold: false
                                },
                                ...
                                },/// ubicacion de realizacion de contrato
                                {
                                    text: [
                        ...
                        {
                            separator: ['2.8.1.', '.'],
                            ol: listaDeObligacionesAMostrar,// datos ingresados desde el formulario 
        ...
        // se crea el pdf con pdfmake y se descarga
        const pdfDocGenerator = pdfMake.createPdf(pdfDocumet);
        pdfDocGenerator.download('Estudio previo ' + contenidoFormulario.tipoContrato + ' numero ' + contenidoFormulario.numeroPrecontractual + '.pdf');
    }
}
console.log("buscar y genear pdf");
let formularioBusquedaEstudio = document.getElementById("busquedaEstudio");
formularioBusquedaEstudio.addEventListener("submit", function (e) {
    e.preventDefault();
    const estudio = document.getElementById("numeroEstudioPrevio").value
    generardocumento(estudio);
})